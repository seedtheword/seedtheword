...
- Telegram: response returns `telegram_sent` / `telegram_skipped`. Dedup now only blocks an **accidental rapid double-submit** of the same subject within `ANNOUNCEMENT_DEDUP_WINDOW_MS` (3 min) via `hasAnnouncementBeenSentRecently_` — a deliberate re-post later in the day is allowed (previously it blocked same-subject for the whole day). **Overrides that skip the dedup entirely:** a super-admin passing `force_telegram:true`, or `urgent`/`emergency` priority (always go out). The composer offers super-admins a one-tap "force send" confirm when Telegram is skipped. **`TELEGRAM_BOT_TOKEN` must hold the `news_seedtheword_bot` token (NOT the Bible bot).** Use `showTelegramTokenStatus` to verify; it warns if it matches `BIBLE_BOT_TOKEN`.
...
- **Photo attachments on announcements:** `handlePostAnnouncement_` now accepts `photo_urls` (array, up to 10; usually one) or `photo_url` (single). Photos are uploaded first via the existing `uploadImage` action (`handleUploadImage_` in `social-handler.gs` → Drive, public URL). When photos are present and audience is Public, they go to Telegram via new helper **`sendTelegramPhotosFromAppsScript_`** (single → `/sendPhoto`, multiple → `/sendMediaGroup`, caption = subject+body, ≤1000 chars on the first item; falls back to `sendMessage` if no URLs). The **first** photo is also written to the community-mirror post's `media_url` so it shows in the feed (feed renders one image; all photos go to Telegram). Photos require the Public audience.
- **Permissions:** `moderation` added to the backend `ALL_PERMISSIONS` whitelist in `order-handler.gs` (and the inline fallback in `handleSetMemberPermissions_`) so the Content Studio "Moderation" toggle now persists on save (it was previously filtered out).

**Frontend (git):** composer gains a 📧 Email/newsletter audience checkbox, a "Recently posted" history panel (so the team doesn't repost), and a **🖼️ Add photo** picker (multi-select, thumbnail previews, remove). Portal has a persistent **"📣 See recent announcements"** link that opens this week's list even after the banner is dismissed. `moderation` added to frontend permission arrays (nav-auth.js, team-portal.js, team.html).
